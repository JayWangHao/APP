...
public class ReplyAdapter extends BaseAdapter {
	...
	public ReplyAdapter(Context context,List<ReplyBean> list,int resourceId,Handler handler,int commentPosition){
...
//		final String commentNickName = bean.getCommentNickname();
		replyNickName = bean.getReplyNickname();
		commentNickName = bean.getCommentNickname();
		replyId = bean.getReplyUserId();
		beReplyId = bean.getCommentUserId();
		...
		//为回复的人昵称添加点击事件
		ss.setSpan(new TextSpanClick(true), 0, replyNickName.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		//为评论的人的添加点击事件
		ss.setSpan(new TextSpanClick(false), replyNickName.length()+2, replyNickName.length()+ commentNickName.length()+2, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
...
//			String msgStr = "";、
			ReplyPostBean bean = new ReplyPostBean();
			bean.setCommentPosition(commentPosition);
			if(status){
//				msgStr = "我是回复的人";
//				bean.setBeReplyUesrId(replyId);
//				bean.setBeReplyName(replyNickName);
//				handler.sendMessage(handler.obtainMessage(14,bean));

			}else{
//				msgStr = "我是被回复的人";
//				bean.setBeReplyUesrId(beReplyId);
//				bean.setBeReplyName(commentNickName);
//				handler.sendMessage(handler.obtainMessage(15,bean));
